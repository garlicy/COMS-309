...
public class CardGraphics {
    ...
    private final int MYPILE = 0, MYPLAY = 2, MYDECK = 3, OPPONENTPILE = 4, OPPONENTPLAY = 5, OPPONENTDECK = 6, SHUFFLE = 7;


    public CardGraphics(){

    }
    public void flipCard(ImageView iv, Drawable d, int place){
    ...
    public void goTo(ImageView iv, int place) {
        ObjectAnimator animX = ObjectAnimator.ofFloat(iv, "translationX", getGoTo(place, true));
        ObjectAnimator animY = ObjectAnimator.ofFloat(iv, "translationY", getGoTo(place, false));
        AnimatorSet animSetXY = new AnimatorSet();
        animSetXY.playTogether(animX, animY);
        animSetXY.start();
        if(place != MYPLAY || place != OPPONENTPLAY)
            assignImage(-1);
    }
    private int getGoTo(int place, boolean getX){
        switch(place){
            case(MYPILE):
                if(getX) return 0;
                return 1;
